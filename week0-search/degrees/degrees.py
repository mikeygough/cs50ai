...
def shortest_path(source, target):
    ...
    # initialize frontier, queue == BFS
    frontier = QueueFrontier()

    # initialize root node (source)
    ...
    while True:
        ...
        if node.state == target:
            movie_person_results = []

            # follow parents to find solution
            while node.parent is not None:
                movie_person = (node.action, node.state)
                movie_person_results.append(movie_person)
                node = node.parent
            movie_person_results.reverse()
            # test
            # print("solution", movie_person_results)
            return movie_person_results
